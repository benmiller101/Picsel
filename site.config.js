...
export const SITE = {
  ...
  /* The default social preview, used by any page that does not set its own.
     ...
     case study should show the site it is about.

     Generated by `npm run og` from the horizontal lockup and this file's own
     copy, not exported by hand. The card it replaced came out of the logo
     suite and read "Websites for Cornwall's trades", which stopped being true
     when the studio started selling across the UK and was invisible from the
     site itself: nobody sees their own share card. The filename carries the
     "-uk" because every scraper caches the old one by URL, so a new card needs
     a new address to be picked up. */
  ogImage: '/assets/brand/open-graph-share-uk.png',
};
