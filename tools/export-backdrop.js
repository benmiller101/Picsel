...
/* ---- Seamless noise, and why there is a second noise function -------------
   ...
   The way you get a repeating 2D field out of noise is to walk a CLOSED LOOP
   through a higher-dimensional one. Going once across the tile means going once
   round a circle, so arriving at the right-hand edge lands you back on the same
   noise you started from and the values match by construction rather than by
   any blending or mirroring — which would soften the field and cost it the
   quantised, clustered character that is the whole effect.

   Two axes need two independent circles, so four dimensions. Hence a 4D simplex
   here rather than reusing the 3D one: sampling a torus embedded in 3D would
   also close both loops, but a torus whose two radii are forced equal by a
   square tile is degenerate at its centre and the pattern smears badly along
   one axis. The 4D version has no such trade — both circles are the same size
   and neither is distorted.

   Seeded identically to the site's field, so the tile is the same texture and
   not merely a similar one. */
function makeNoise4D(seed) {
