...
/* ---- robots.txt -----------------------------------------------------------
   ...
   The policy itself is in ROBOTS in site.config.js, because who may crawl the
   site is a fact about the studio rather than about the build. This function
   only renders it. Everything is allowed; the reasoning, including why there
   are no Disallow lines anywhere, is written down beside the data.

   ONE GROUP PER USER AGENT, ENFORCED BELOW. Until 17 August 2026 Cloudflare
   served a managed robots.txt in front of this one, and between them several
   user agents were named twice. A crawler obeys the most specific group that
   matches it, and crawlers disagree about what to do when two are equally
   specific: some take the first, some the last, some merge them. A file that
   means different things to different readers cannot be reasoned about, which
   is worse than one that is merely restrictive. The managed file is off now
   and this is the only robots.txt served, so a duplicate can only come back
   from this repo. The build stops it. */
async function writeRobots() {
  const { contentSignal, crawlers } = ROBOTS;

  /* A blank line between the groups. robots.txt does not require it; it is
     here so the file is readable by the person checking it, which is the only
     reason anybody ever opens a robots.txt by hand. */
  const BLANK_LINE = '\n';

  /* The wildcard group goes first because a crawler with no group of its own
     has to find it, and Content-Signal rides on it so that it covers the
     crawlers nobody has named yet as well as the ones below. */
...
`,
  ...
  /* The check that turns the paragraph above from an intention into a fact.
     Case-insensitive, because robots.txt user-agent matching is: "GPTBot" and
     "gptbot" are one crawler to a crawler and two entries to anybody skimming
     the list, which is the worst of both.

     It throws rather than warns. A warning in a build that writes the file
     anyway is a warning nobody reads until a crawler has already made its own
     decision about which of the two groups to obey. */
  const seen = new Set();
  for (const { agent } of [{ agent: '*' }, ...crawlers]) {
    const key = agent.toLowerCase();
    if (seen.has(key)) {
      throw new Error(
        `robots.txt: "${agent}" appears more than once in ROBOTS.crawlers. One `
        + 'group per user agent: crawlers parse duplicates inconsistently, so a '
        + 'second group makes the file mean different things to different '
        + 'readers. Merge them in site.config.js.',
      );
    }
    seen.add(key);
  }
...
#
# One group per user agent, checked at build time. Everything is allowed and
# nothing is disallowed: /contact/sent/ is kept out of search with a noindex
# tag, which only works if the page can be crawled and read.

${wildcard}
# Named individually so that restricting the wildcard above can never cut them
# off by accident: a crawler that matches its own group ignores the wildcard
# group entirely. Picsel sells being the answer an assistant gives.
${groups.join(BLANK_LINE)}
